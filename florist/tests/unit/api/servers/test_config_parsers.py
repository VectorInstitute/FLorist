...
def test_parse_config_success() -> None:
    ...
    config_parser = ConfigParser.class_for_parser(ConfigParser.FEDAVG)
    result = config_parser.parse(json.dumps(test_config))

    assert result == test_config


def test_parse_config_fail_missing_required_field() -> None:
...
def test_parse_config_fail_not_json() -> None:

...
def test_train():
    ...
    server_config = uvicorn.Config(
        "florist.api.server:app", host="localhost", port=8000, log_level="debug")
    server_service = TestUvicornServer(config=server_config)
    client_config = uvicorn.Config(
        "florist.api.client:app", host="localhost", port=8001, log_level="debug")
        ...
        with client_service.run_in_thread():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                assert response.json() == {
                    "server_uuid": ANY, "client_uuids": [ANY]}

                redis_conn = redis.Redis(
                    host=test_redis_host, port=test_redis_port)
                ...
                assert server_metrics_result is not None and isinstance(
                    server_metrics_result, bytes)
                ...
                assert client_metrics_result is not None and isinstance(
                    client_metrics_result, bytes)
                client_metrics = json.loads(
                    client_metrics_result.decode("utf8"))

...
class RedisMetricsReporter(MetricsReporter):
    """
    Saves the metrics to a Redis instance while it records them.
    """
    ...
    def add_to_metrics(self, data: Dict[str, Any]) -> None:
        """
        Adds a dictionary of data into the main metrics dictionary. At the end, dumps the current
        state of the metrics to Redis.
    ...
    def add_to_metrics_at_round(self, fl_round: int, data: Dict[str, Any]) -> None:
        """
        Adds a dictionary of data into the metrics dictionary for a specific FL round. At the end,
        dumps the current state of the metrics to Redis.
    ...
    def dump(self) -> None:
        """
        Dumps the current metrics to Redis under the run_id name.
        """

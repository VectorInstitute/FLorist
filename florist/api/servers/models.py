...
class ServerFactory:
    def __init__(
        self,
        get_server_function: Callable[[nn.Module, int, list[BaseReporter], dict[str, Any]], FlServer],
    ):
...
class Model(Enum):
    ...
    def server_factory_for_model(cls, model: Self) -> ServerFactory:
        ...
        """
        if model == Model.MNIST_FEDAVG:
            return ServerFactory(get_server_function=get_fed_avg_server)
        # TODO
        # if model == Model.MNIST_FEDPROX:
        #     return ConfigParser.FEDPROX
